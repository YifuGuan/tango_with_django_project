...
class Chapter7FormClassTests(TestCase):
    ...
    def test_category_form_class(self):
        ...
        import rango.forms
        self.assertTrue('CategoryForm' in dir(
            rango.forms), f"{FAILURE_HEADER}The class CategoryForm could not be found in Rango's forms.py module. Check you have created this class in the correct location, and try again.{FAILURE_FOOTER}")

        from rango.forms import CategoryForm
        category_form = CategoryForm()

        # Do you correctly link Category to CategoryForm?
        self.assertEqual(type(category_form.__dict__[
                         'instance']), Category, f"{FAILURE_HEADER}The CategoryForm does not link to the Category model. Have a look in the CategoryForm's nested Meta class for the model attribute.{FAILURE_FOOTER}")
        ...
        for expected_field_name in expected_fields:
            ...
            self.assertEqual(expected_field, type(
                fields[expected_field_name]), f"{FAILURE_HEADER}The field '{expected_field_name}' in CategoryForm was not of the expected type '{type(fields[expected_field_name])}'.{FAILURE_FOOTER}")
...
class Chapter7CategoryFormAncillaryTests(TestCase):
    ...
    def test_add_category_functionality(self):
        ...
        self.assertEqual(len(
            categories), 1, f"{FAILURE_HEADER}When adding a new category, it does not appear in the list of categories after being created. Check your add_category() view as the start of a debugging point.{FAILURE_FOOTER}")
...
class Chapter7PageFormClassTests(TestCase):
    ...
    def test_page_form_class(self):
        ...
        import rango.forms
        self.assertTrue('PageForm' in dir(
            rango.forms), f"{FAILURE_HEADER}The class PageForm could not be found in Rango's forms.py module. Check you have created this class in the correct location, and try again.{FAILURE_FOOTER}")

        from rango.forms import PageForm
        page_form = PageForm()

        # Do you correctly link Page to PageForm?
        self.assertEqual(type(page_form.__dict__[
                         'instance']), Page, f"{FAILURE_HEADER}The PageForm does not link to the Page model. Have a look in the PageForm's nested Meta class for the model attribute.{FAILURE_FOOTER}")
        ...
        for expected_field_name in expected_fields:
            ...
            self.assertEqual(expected_field, type(
                fields[expected_field_name]), f"{FAILURE_HEADER}The field '{expected_field_name}' in PageForm was not of the expected type '{type(fields[expected_field_name])}'.{FAILURE_FOOTER}")
...
class Chapter7PageFormAncillaryTests(TestCase):
    ...
    def test_add_page_url_mapping(self):
        ...
        """
        try:
            resolved_url = reverse('rango:add_page', kwargs={
    ...
    def test_add_page_form_response(self):
        ...
        populate()
        response = self.client.get(reverse('rango:add_page', kwargs={
                                   'category_name_slug': 'django'}))
        context = response.context
        content = response.content.decode()

        self.assertTrue(
            '<form' in content, f"{FAILURE_HEADER}We couldn't find a <form> element in the response for adding a page.{FAILURE_FOOTER}")
    ...
    def test_add_page_bad_category(self):
        ...
        """
        response = self.client.get(reverse('rango:add_page', kwargs={
                                   'category_name_slug': 'non-existent'}))
    ...
    def test_add_page_functionality(self):
        ...
        self.assertEqual(len(
            python_pages), 1, f"{FAILURE_HEADER}When adding a new page to a category with the add_page form, the new Page object that we were expecting wasn't created. Check your add_page() view for mistakes, and try again. You need to call .save() on the page you create!{FAILURE_FOOTER}")
