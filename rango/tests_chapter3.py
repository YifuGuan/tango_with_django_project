...
class Chapter3ProjectStructureTests(TestCase):
    ...
    def test_project_created(self):
        ...
        directory_exists = os.path.isdir(os.path.join(
            self.project_base_dir, 'tango_with_django_project'))
        urls_module_exists = os.path.isfile(os.path.join(
            self.project_base_dir, 'tango_with_django_project', 'urls.py'))

        self.assertTrue(
            directory_exists, f"{FAILURE_HEADER}Your tango_with_django_project configuration directory doesn't seem to exist. Did you use the correct name?{FAILURE_FOOTER}")
        self.assertTrue(
    ...
    def test_rango_app_created(self):
        ...
        self.assertTrue(
            directory_exists, f"{FAILURE_HEADER}The rango app directory does not exist. Did you use the startapp command?{FAILURE_FOOTER}")
        self.assertTrue(
            is_python_package, f"{FAILURE_HEADER}The rango directory is missing files. Did you use the startapp command?{FAILURE_FOOTER}")
        self.assertTrue(
            views_module_exists, f"{FAILURE_HEADER}The rango directory is missing files. Did you use the startapp command?{FAILURE_FOOTER}")
    ...
    def test_rango_has_urls_module(self):
        ...
        module_exists = os.path.isfile(
            os.path.join(self.rango_app_dir, 'urls.py'))
        self.assertTrue(
            module_exists, f"{FAILURE_HEADER}The rango app's urls.py module is missing. Read over the instructions carefully, and try again. You need TWO urls.py modules.{FAILURE_FOOTER}")
    ...
    def test_is_rango_app_configured(self):
        ...
        self.assertTrue(
            is_app_configured, f"{FAILURE_HEADER}The rango app is missing from your setting's INSTALLED_APPS list.{FAILURE_FOOTER}")
...
class Chapter3IndexPageTests(TestCase):
    ...
    def test_view_exists(self):
        ...
        self.assertTrue(
            name_exists, f"{FAILURE_HEADER}The index() view for rango does not exist.{FAILURE_FOOTER}")
        self.assertTrue(
...
class Chapter3AboutPageTests(TestCase):
    ...
    def test_view_exists(self):
        ...
        self.assertTrue(
            name_exists, f"{FAILURE_HEADER}We couldn't find the view for your about view! It should be called about().{FAILURE_FOOTER}")
        self.assertTrue(
            is_callable, f"{FAILURE_HEADER}Check you have defined your about() view correctly. We can't execute it.{FAILURE_FOOTER}")
